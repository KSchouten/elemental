ElementalModuleSettings <- R6::R6Class(
  ...
  private = list(
    ...
    server = function(input, output, session){
      ...
      output$modal_ui <- renderUI({
        tagList(
          h1(private$globals$i18n$ui_t("Module settings")),
          p(private$module()$get_id()),
          
          textInput(ns("title"), private$globals$i18n$ui_t("Title"), private$module()$get_title()),
          if(length(private$module()$get_params())>0){
            tagList(
              h4(private$globals$i18n$ui_t("Parameters")),
          ...
          if(length(private$module()$get_inputs())>0){
            tagList(
              h4(private$globals$i18n$ui_t("Dependencies")),
  ...
  public = list(
    ...
    initialize = function(id, title, globals, tile, module){
      ...
      private$all_available_exports <- append(list("") %>% setNames(private$globals$i18n$static_t("Choose value from another module")), purrr::map(private$globals$modules, function(m){
